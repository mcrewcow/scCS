...
- Rebuilt scCS around a supervised furcation model that keeps scientific
  RNA-velocity dynamics separate from the standardized star display.
- Added two explicit scoring modes: instantaneous supervised-geometry
  scoring and Discounted Future-Fate Propagation (DFFP).
- Preserved SingleScorer, PairScorer, and MultiScorer as the principal
  public analysis interfaces.

**Discounted Future-Fate Propagation**

- Added discounted propagation on the original RNA-velocity transition
  graph.
- Added Conditional Fate Affinity (CFA), Discounted Fate Reach (DFR),
  Future-Fate Specificity (FFS), Resolved Commitment (RC), Unresolved
  Future Probability (UFP), and Signed Ordering Flux (SOF).
- Added optional explicit competing outcomes, endpoint-anchor diagnostics,
  horizon sensitivity, and transition-normalization diagnostics.
- Kept fate identity, fate resolution, decisiveness, and forward or
  retrograde progression as separate quantities.

**Analysis and inference**

- Added replicate-aware two-condition inference through PairScorer.
- Added omnibus tests, multiplicity-controlled post-hoc comparisons, and
  prespecified contrasts through MultiScorer.
- Added hierarchical bootstrap, exact replicate-label permutation,
  optional mixed-model sensitivity, and fail-closed inference diagnostics.
- Added commitment-associated gene analysis, fate-marker analysis,
  expression trends, enrichment, and reproducible result export.

**Visualization**

- Added standardized root-plus-star visualizations for future-fate
  affinity, reach, entropy, specificity, resolved commitment, and signed
  progression.
- Added gene-expression stars, annotated heatmaps, condition grids,
  branch-relative velocity profiles, root fate-directed rose plots, and
  display-only scVelo velocity projection.
- Added explicit support for turning, loop-like, mixed, and retrograde
  trajectories without forcing all branches to move monotonically outward.

**Documentation and tutorials**

- Added complete pancreas and RegVelo Schwann tutorials for SingleScorer,
  PairScorer, and MultiScorer.
- Added mathematical, method-selection, complex-branch, downstream-analysis,
  package-validation, and no-chunk scalability tutorials.
- Added a first-time-user introduction, mathematical framework, terminology,
  diagnostics, interpretation guidance, and Read the Docs navigation.

**Validation and scalability**

- Validated DFFP on pancreas and RegVelo Schwann using preserved source
  velocity graphs.
- Added synthetic controls for leave-and-return paths, competing outcomes,
  loops, retrograde motion, and solver equivalence.
- Added complete single-process scalability benchmarks without replacing
  scientific scoring with chunked calculations.
- Added comprehensive package, documentation, distribution, and H5AD
  persistence tests.

**Breaking changes**

- v0.8.0 is a clean redesign of the v0.7 implementation. Several v0.7
  internal modules and result fields were replaced.
- Users should follow the v0.8 tutorials when migrating existing analyses.
